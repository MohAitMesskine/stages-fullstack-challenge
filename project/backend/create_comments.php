...
/**
 * Script pour créer automatiquement des commentaires via Tinker
 * 
 * Utilisation:
 * docker-compose exec backend php artisan tinker
 * 
 * Puis collez ce code:
 * require 'create_comments.php';
 * 
 * Ou directement:
 * docker-compose exec backend php create_comments.php
 */
...
// Demander combien de commentaires par article
echo "💬 Combien de commentaires voulez-vous créer par article ? (défaut: 3-8 aléatoire): ";
$input = trim(fgets(STDIN));
$commentsPerArticle = $input ? (int)$input : null;

$totalCreated = 0;

foreach ($articles as $article) {
    // Nombre aléatoire de commentaires entre 3 et 8 si non spécifié
    $numComments = $commentsPerArticle ?? rand(3, 8);
